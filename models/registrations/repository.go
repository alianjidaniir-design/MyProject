...
func (repo *Repository) Cancel(ctx context.Context, req commonSchema.BaseRequest[registrationSchema.GetRegisteredStudentsRequest], c *fiber.Ctx) (res registrationSchema.CancelRegistrationResponse, errStr string, code int, err error) {
	if repo.initRepo != nil {
	...
	role := authz.GetRoleName(c)
	studentID := authz.GetUserID(c)
	cancel, err := repo.db().CancelRegisterStudent(ctx, req.Body, role, studentID)
	if err != nil {
