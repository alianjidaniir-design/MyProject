...
func (repo *Repository) ListDepartment(ctx context.Context, req commonSchema.BaseRequest[courseSchema.DepartmentListRequest]) (res courseSchema.CourseListResponse, errStr string, code int, err error) {
	if repo.initErr != nil {
		return courseSchema.CourseListResponse{}, "01", status.StatusUnauthorized, repo.initErr
	}
	if repo.DBDS == nil {
		return courseSchema.CourseListResponse{}, "02", status.StatusBadRequest, fmt.Errorf("DBDS is nil")
	}
	list, totalAll, err := repo.db().ListDepartmentsCourse(ctx, req.Body)
	if err != nil {
		return courseSchema.CourseListResponse{}, "03", status.StatusInternalServerError, err
	}
	return courseSchema.CourseListResponse{Courses: list, Total: totalAll}, "", status.StatusOK, nil
}
