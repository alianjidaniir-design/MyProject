...
func ListSummeryStudents(ctx *fiber.Ctx) error {
	spanCtx := mainController.InitAPI(ctx, "15")
	defer mainController.FinishSpan(ctx)
	req := commonSchema.BaseRequest[profileSchema.ListAllScoresReq]{}
	errStr, code, err := mainController.ParseBody(ctx, &req)
	if err != nil {
		return mainController.Error(ctx, controllerbaseErrCode.ProfileErrCode, "01", errStr, code, err)
	}
	res, errStr, code, err := repositories.ProfileRepo.Summery(spanCtx, req)
	if err != nil {
