...
func ListOffering(ctx *fiber.Ctx) error {
	spanCtx := mainController.InitAPI(ctx, "14")
	defer mainController.FinishSpan(ctx)
	req := commonSchema.BaseRequest[registrationSchema.ListOfferingRequest]{}
	errStr, code, err := mainController.ParseBody(ctx, &req)
	if err != nil {
		return mainController.Error(ctx, controllerbaseErrCode.RegistrationErrCode, "01", errStr, code, err)
	}
	res, errStr, code, err := repositories.RegistrationRepo.ListOfferings(spanCtx, req)
	if err != nil {
